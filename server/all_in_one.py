...
#     # 参数校验
#     if not (user_id and chat_id and title):
#         return jsonify({
#             'success': False,
#             'message': '必须提供 user_id/chat_id/title'
#         }), 400
...
#     new_chat = ChatRecord(user_id=user_id, message=message)
#     db.session.add(new_chat)
#     db.session.commit()
...
#     chats = ChatRecord.query.filter_by(user_id=user_id).order_by(ChatRecord.timestamp).all()
#     chat_list = [{'message': chat.message, 'timestamp': chat.timestamp} for chat in chats]
